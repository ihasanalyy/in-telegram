...
async function premiumCardCreationUsingWallet(chatId, payload, chat, text, selectedLanguage, data, image_payloads, video_payloads) {
    // user is shown list of wallets
    if (payload === "activate_vcc_v_p_a_ip_w"
        && (chat?.last_message === "activate_vcc_v_p_a_payment_method"
            || chat?.last_message === "activate_vcc_v_p_a_ip_w_wallets"
            || chat?.last_message === "activate_vcc_v_p_a_ip_w_confirm"
            || chat?.last_message === "activate_vcc_v_p_a_ip_w_balance_insufficient"
            || chat?.last_message === "activate_vcc_v_p_a_ip_w_proceed")
    ) {
        ...
        await sendButtons(chatId, lang[selectedLanguage].SELECT_WALLET_CURRENCY, buttons, "activate_vcc_v_p_a_ip_w_wallets");
    }
    // user has selected a currency
    else if (payload?.includes('activate_vcc_v_p_a_ip_w-') && chat?.last_message === "activate_vcc_v_p_a_ip_w_wallets") {
        ...
        const buttons = [
            [{ text: lang[selectedLanguage].CHANGE_WALLET, callback_data: "activate_vcc_v_p_a_ip_w" }],
            [{ text: lang[selectedLanguage].PROCEED_TITLE, callback_data: "activate_vcc_v_p_a_ip_w_proceed" }],
            [{ text: lang[selectedLanguage].MAIN_MENU, callback_data: "main_menu" }]
        ];

        await sendButtons(chatId, message, buttons, "activate_vcc_v_p_a_ip_w_proceed");
    }
    // user has selected proceed
    else if (payload === "activate_vcc_v_p_a_ip_w_proceed" && chat?.last_message === "activate_vcc_v_p_a_ip_w_proceed") {
...
💰 Fee: ${formattedAmount(convertedFee)} ${walletDetails.currency.code} `
        ...
        const buttons = [
            [{ text: lang[selectedLanguage].CONFIRM, callback_data: "activate_vcc_v_p_a_ip_w_confirm" }],
            [{ text: lang[selectedLanguage].CHANGE_WALLET, callback_data: "activate_vcc_v_p_a_ip_w" }],
            [{ text: lang[selectedLanguage].MAIN_MENU, callback_data: "main_menu" }]
        ];

        await sendButtons(chatId, message, buttons, "activate_vcc_v_p_a_ip_w_confirm");

    }
    // user has confirmed, we will check the balance validation too
    else if (payload === "activate_vcc_v_p_a_ip_w_confirm" && chat?.last_message === "activate_vcc_v_p_a_ip_w_confirm") {
        const walletDetails = await getActiveWalletById(chat.vcc.wallet);
        ...
        if (walletDetails.balance.available < convertedFee) {
            ...
            await sendButtons(chatId, lang[selectedLanguage].INSUFFICIENT_BALANCE, buttons, "activate_vcc_v_p_a_ip_w_balance_insufficient");
        } else {
            await handleOTPGenerationTG(selectedLanguage, chat, "activate_vcc_v_p_a_ip_w-otp", "activate_vcc_v_p_a_ip_w-otp");
        }
    }

    else if (chat.last_message === "activate_vcc_v_p_a_ip_w-otp" && text && !payload) {
        const otpValidationResult = await validateOTPTG(chatId, text, "activate_vcc_v_p_a_ip_w-otp");
...
`
            ...
            if (otpValidationResult.message === "max_attempts_exceeded") {
                ...
                await invalidMessageTG(chatId, "activate_vcc_v_p_a_ip_w-otp", selectedLanguage, chat?.otpType);
            }
        }
    }
}

module.exports = { premiumCardCreationUsingWallet }