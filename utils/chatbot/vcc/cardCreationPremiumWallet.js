...
const { balanceLimitCheck, limitCheck, featureCheck, getExchangeRatesToUSD, fetchLocalOrDefaultWalletConditionally, getUserActiveWallets, getActiveWalletById, getMaxValidExpiryDateVCC } = require("../../helpers");
...
async function premiumCardCreationUsingWallet(senderId, payload, account, bot, text, selectedLanguage) {
    const data = {
        sender: {
            id: senderId
        }
    ...
    if (payload === "activate_vcc_v_p_a_ip_w") {
        ...
        let quickReplies = slicedWallets.map((wallet) => ({
            ...
            payload: `activate_vcc_v_p_a_ip_w-${wallet._id}`
        }));
        ...
        await quickReply(data, lang[selectedLanguage].SELECT_WALLET_CURRENCY, quickReplies, "activate_vcc_v_p_a_ip_w_wallets");
    }
    // User has selected a currency
    else if (payload?.includes('activate_vcc_v_p_a_ip_w-') && bot?.last_message === "activate_vcc_v_p_a_ip_w_wallets") {
        ...
        const quickReplies = [
            {
                content_type: "text",
                title: lang[selectedLanguage].CHANGE_WALLET,
                payload: "activate_vcc_v_p_a_ip_w"
            },
            {
                content_type: "text",
                title: lang[selectedLanguage].PROCEED_TITLE,
                payload: "activate_vcc_v_p_a_ip_w_proceed"
            },
            {
                content_type: "text",
        ...
        await quickReply(data, message, quickReplies, "activate_vcc_v_p_a_ip_w_proceed");
    }
    // User has selected proceed
    else if (payload === "activate_vcc_v_p_a_ip_w_proceed" && bot?.last_message === "activate_vcc_v_p_a_ip_w_proceed") {
        ...
        const vvcFee = await FeeModel.findOne({
            $and: [
                ...
                { account_level: account.level._id }
            ]
        });

        const convertedFee = formatDecimalNumbersWithLimit(
...
💰 Fee: ${formattedAmount(convertedFee)} ${walletDetails.currency.code}`;
        ...
        const quickReplies = [
            {
                content_type: "text",
                title: lang[selectedLanguage].CONFIRM,
                payload: "activate_vcc_v_p_a_ip_w_confirm"
            },
            {
                content_type: "text",
                title: lang[selectedLanguage].CHANGE_WALLET,
        ...
        await quickReply(data, message, quickReplies, "activate_vcc_v_p_a_ip_w_confirm");
    }
    // User has confirmed, we will check the balance validation too
    else if (payload === "activate_vcc_v_p_a_ip_w_confirm" && bot?.last_message === "activate_vcc_v_p_a_ip_w_confirm") {
        const walletDetails = await getActiveWalletById(bot.vcc.wallet);
        ...
        if (walletDetails.balance.available < convertedFee) {
            const quickReplies = [
                { content_type: "text", title: lang[selectedLanguage].CHANGE_WALLET, payload: "activate_vcc_v_p_a_ip_w" },
                { content_type: "text", title: lang[selectedLanguage].MAIN_MENU, payload: "main_menu" }
            ];

            await quickReply(data, lang[selectedLanguage].INSUFFICIENT_BALANCE, quickReplies, "activate_vcc_v_p_a_ip_w_balance_insufficient");
        } else {
            await handleOTPGeneration(selectedLanguage, senderId, "activate_vcc_v_p_a_ip_w-otp", "activate_vcc_v_p_a_ip_w-otp", "Transaction OTP");
        }
    }

    // User has entered OTP
    else if (bot.last_message === "activate_vcc_v_p_a_ip_w-otp" && text && !payload) {
        const otpValidationResult = await validateOTP(senderId, text, "activate_vcc_v_p_a_ip_w-otp");
        ...
        if (otpValidationResult.status) {
            ...
            const data = {
                ...
                cardType: serviceName,
            };

            if (bot.vcc?.isPremiumPlus) {
                data.apple_pay = true;
                data.google_pay = false;
            }
                    ...
                    elements: [
                        {
                            ...
                            image_url: "https://nodejs-checking-bucket.s3.eu-west-3.amazonaws.com/telegram_bot_images/Premium1.png",
                            buttons: [
                                {
                                    type: "postback",
            ...
            if (otpValidationResult.message === "max_attempts_exceeded") {
                ...
                await invalidMessage(data, "activate_vcc_v_p_a_ip_w-otp", selectedLanguage, bot?.otpType);
            }
        }
    }
}
...
module.exports = {
    premiumCardCreationUsingWallet
}