...
async function intlUsingPaypal(senderId, payload, account, bot, text, selectedLanguage, attachments) {
    ...
    if (payload === "intl_paypal_payment") {

        const message = lang[selectedLanguage].ENTER_AMOUNT_CURRENCY.replace('{{currency}}', defaultWallet?.currency.code);

        await quickMessage(data, message, "intl_paypal_payment_amount");
    }

    // user has entered amount
    if (bot?.last_message === "intl_paypal_payment_amount" && !payload && text) {
        console.log(text)
...
${lang[selectedLanguage].EXCHANGE_RATE}: 1.00 ${rates?.sending?.currency} = ${formattedAmount(rates?.paypal?.paypal_rate.value)} ${rates?.paypal?.paypal_rate.currency}
...
`;
                ...
                const quickReplies = [
                    { content_type: "text", title: lang[selectedLanguage].PROCEED_TO_TRANSFER, payload: "intl_paypal_payment_proceed_transfer" },
                    { content_type: "text", title: lang[selectedLanguage].ADJUST_AMOUNT_TITLE, payload: "intl_paypal_payment_adjust_amount" },
    ...
    else if (payload === "intl_paypal_payment_adjust_amount") {
        const quickReplies = [
            { content_type: "text", title: lang[selectedLanguage].MAIN_MENU, payload: "main_menu" },
        ]
        await quickReply(data, lang[selectedLanguage].ENTER_AMOUNT_DIGITS, quickReplies, "intl_paypal_payment_amount");
    }
    // user has proceeded with the details
    else if (payload === "intl_paypal_payment_proceed_transfer") {
        if (account?.level?.level_no === 1) {
            await userKYCVerificationTemplate(data, data?.sender?.id, selectedLanguage)
        } else {
            const quickReplies = [
                { content_type: "text", title: lang[selectedLanguage].PERSONAL_SUPPORT, payload: "intl_paypal_payment_purpose-FAMILY_SUPPORT" },
                { content_type: "text", title: lang[selectedLanguage].EDUCATION, payload: "intl_paypal_payment_purpose-EDUCATION" },
                { content_type: "text", title: lang[selectedLanguage].MEDICAL_TREATMENTS, payload: "intl_paypal_payment_purpose-MEDICAL_TREATMENT" },
                { content_type: "text", title: lang[selectedLanguage].OPERATIONAL_COSTS, payload: "intl_paypal_payment_purpose-SERVICE_CHARGES" },
                { content_type: "text", title: lang[selectedLanguage].CHARITY_DONATIONS, payload: "intl_paypal_payment_purpose-GIFT_AND_DONATION" },
                { content_type: "text", title: lang[selectedLanguage].OTHER_REASONS, payload: "intl_paypal_payment_purpose-OTHER" },
                { content_type: "text", title: lang[selectedLanguage].MAIN_MENU, payload: "main_menu" },
            ];
    ...
    // user has selected a purpose
    else if (payload?.includes("intl_paypal_payment_purpose-")) {
        ...
        const quickReplies = [
            ...
            { content_type: "text", title: lang[selectedLanguage].ADD_NOTE_BUTTON, payload: "intl_paypal_payment_note" },
            { content_type: "text", title: lang[selectedLanguage].ATTACH_DOCUMENT_BUTTON, payload: "intl_paypal_payment_doc" },
            { content_type: "text", title: lang[selectedLanguage].SKIP, payload: "intl_paypal_payment_without_doc" },
        ];
    ...
    else if (payload === "intl_paypal_payment_note") {
        await quickMessage(data, lang[selectedLanguage].PLEASE_DESCRIBE_PURPOSE, "intl_paypal_payment_note");
    }
    // user has entered a note
    else if (bot?.last_message === "intl_paypal_payment_note" && text && !payload) {

        bot.intl.intl_note = text
        await bot.save()

        const quickReplies = [
            { content_type: "text", title: lang[selectedLanguage].YES, payload: "intl_paypal_payment_add_attch" },
            { content_type: "text", title: lang[selectedLanguage].NO, payload: "intl_paypal_payment_no_attch" },
        ]
    ...
    else if (payload === "intl_paypal_payment_add_attch") {
        const quickReplies = [
            { content_type: "text", title: lang[selectedLanguage].SKIP, payload: "intl_paypal_payment_no_attch" },
            { content_type: "text", title: lang[selectedLanguage].CANCEL, payload: "main_menu" },
        ]

        await quickReply(data, lang[selectedLanguage].ATTACH_DOCUMENT_PROMPT, quickReplies, "intl_paypal_payment_attachments");
    }
    // user has not proceeded with adding an attachement
    else if (payload === "intl_paypal_payment_no_attch" || payload === "intl_paypal_payment_no_note" || payload === "intl_paypal_payment_without_doc") {
        await processIntlProceedTransfer(data, bot, account, bot?.intl_country_code, "intl_paypal_payment", selectedLanguage);
    }

    else if (attachments?.attachments && bot?.last_message === "intl_paypal_payment_attachments" && !payload) {
    ...
    else if ((attachments?.is_unsupported || text) && bot?.last_message === "intl_paypal_payment_attachments" && !payload) {
        await quickMessage(data, lang[selectedLanguage].INVALID_ATTACHMENT_TYPE,);
    }
    // user has attached an image as a document
    else if (payload === "intl_paypal_payment_doc") {
        await quickMessage(data, lang[selectedLanguage].ATTACH_DOCUMENT_PROMPT, "intl_paypal_payment_attachments1");
    }

    else if (attachments?.attachments && bot?.last_message === "intl_paypal_payment_attachments1" && !payload) {
        const { validCount, allImagesAndVideos } = validateAttachments(attachments.attachments);
        ...
        if (allImagesAndVideos) {
            ...
            if (uploadedImages.status) {
                ...
                await bot.save();
                const quickReplies = [
                    { content_type: "text", title: lang[selectedLanguage].YES, payload: "intl_paypal_payment_add_note" },
                    { content_type: "text", title: lang[selectedLanguage].NO, payload: "intl_paypal_payment_no_note" },
                ]

                await quickReply(data, lang[selectedLanguage].ATTACH_NOTE_PROMPT, quickReplies, "4");
            } else {
                const quickReplies = [
                    { content_type: "text", title: lang[selectedLanguage].MAIN_MENU, payload: "main_menu" },
                ];
    ...
    else if ((attachments?.is_unsupported || text) && bot?.last_message === "intl_paypal_payment_attachments1" && !payload) {
        await quickMessage(data, lang[selectedLanguage].INVALID_ATTACHMENT_TYPE,);
    }
    // user has proceeded with additional note request
    else if (payload === "intl_paypal_payment_add_note") {
        const quickReplies = [
            { content_type: "text", title: lang[selectedLanguage].SKIP, payload: "intl_paypal_payment_no_attch" },
            { content_type: "text", title: lang[selectedLanguage].CANCEL, payload: "main_menu" },
        ]
        await quickReply(data, lang[selectedLanguage].PLEASE_DESCRIBE_PURPOSE, quickReplies, "intl_paypal_payment_note1");

        // await quickMessage(data,  lang[selectedLanguage].PLEASE_DESCRIBE_PURPOSE, "intl_paypal_payment_note1");
    }
    // has added the note
    else if (bot?.last_message === "intl_paypal_payment_note1" && text && !payload) {
        bot.intl.intl_note = text
        await bot.save()
        // show beneficiaries here
        await processIntlProceedTransfer(data, bot, account, bot?.intl_country_code, "intl_paypal_payment", selectedLanguage);
    }
    else if (payload?.includes("intl_paypal_payment_next_beneficiaries")) {
        ...
        const beneficiaryList = beneficiariesToDisplay.map((beneficiary, index) => ({
            ...
            payload: `intl_paypal_payment_select_benef_${beneficiary._id}`
        }));
        ...
        if (currentPage === 2) {
            quickReplies.push({ content_type: "text", title: lang[selectedLanguage].PREVIOUS_BUTTON_TITLE, payload: `intl_paypal_payment_no_attch` })
            quickReplies.push({ content_type: "text", title: lang[selectedLanguage].MAIN_MENU_MESSAGE, payload: `main_menu` })
        } else {
            quickReplies.push({ content_type: "text", title: lang[selectedLanguage].PREVIOUS_BUTTON_TITLE, payload: `intl_paypal_payment_prev_beneficiaries_${currentPage}` },)
            quickReplies.push({ content_type: "text", title: lang[selectedLanguage].MAIN_MENU_MESSAGE, payload: `main_menu` })
        ...
        if (beneficiaries.length > endIndex) {
            quickReplies.unshift({ content_type: "text", title: lang[selectedLanguage].NEXT_BUTTON_TITLE, payload: `intl_paypal_payment_next_beneficiaries_${currentPage + 1}` });
        }

        await quickReply(data, message, quickReplies, "4");
    }
    // user is selecting the beneficiary number
    else if (payload?.includes("intl_paypal_payment_select_benef_")) {
        ...
        if (quotationDetails?.status) {
            ...
            const quickReplies = [
                { content_type: "text", title: lang[selectedLanguage].YES_CONTINUE_TITLE, payload: "intl_paypal_payment_confirm_beneficiary" },
                { content_type: "text", title: lang[selectedLanguage].CHANGE_BENEFICIARY, payload: "intl_paypal_payment_no_attch" },
                { content_type: "text", title: lang[selectedLanguage].MAIN_MENU, payload: "main_menu" }
            ];
            await quickReply(data, message, quickReplies, "4");
        }
    ...
    else if (payload === "intl_paypal_payment_confirm_beneficiary") {
...
${rates.total.currency !== rates.exchanged_rate.currency ? `${lang[selectedLanguage].EXCHANGE_RATE}: 1.00 ${rates.total.currency} = ${formatDecimalNumbersWithLimit(formattedAmount(rates.exchanged_rate.value, 6), 6)} ${rates.exchanged_rate.currency}\n` : ''}
...
`;
                ...
                const quickReplies = [
                    { content_type: "text", title: lang[selectedLanguage].CONFIRM_TRANSACTION, payload: "intl_paypal_payment_confirm_transaction" },
                    { content_type: "text", title: lang[selectedLanguage].MAIN_MENU, payload: "main_menu" }
                ];
    ...
    else if (payload === "intl_paypal_payment_confirm_transaction") {
        await handleOTPGeneration(selectedLanguage, senderId, "intl_paypal_payment-otp", "intl_paypal_payment-otp", "Transaction OTP");
    }
    // user has entered otp
    else if (bot?.last_message === "intl_paypal_payment-otp" && !payload && text) {
        const otpValidationResult = await validateOTP(senderId, text, "intl_paypal_payment-otp");
        ...
        if (otpValidationResult.status) {
            ...
            const paypalTransactionInitiate = await initiateIntlPaypalTransactionHelper(decodedToken, defaultWallet._id, bot.intl.card_payment.intl_exchngrate_token)
...
${lang[selectedLanguage].TOTAL_AMOUNT}: ${formattedAmount(paypalDetails?.paypal_currency_supported ? decodedToken.transactionDetails.calculations?.total.value : paypalDetails?.paypal_converted.value)} ${paypalDetails?.paypal_currency_supported ? defaultWallet?.currency?.code : paypalDetails?.paypal_converted.currency}
        `;
                            ...
                            buttons: [
                                {
                                    ...
                                    url: paypalTransactionInitiate?.url,
                ...
                await somethingWentWrongQuickReply(data, lang[selectedLanguage].TRANSACTION_PROCESSING_ERROR);
            }
...
module.exports = intlUsingPaypal